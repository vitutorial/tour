...
\begin{frame}{Reparameterised gradients: Inverse cdf}
	Inverse cdf
	\begin{itemize}
...
\begin{frame}{Beyond}
	...
	Many interesting densities cannot be easily reparameterised %are not location-scale families
...
\begin{frame}{VI for Weibull-Poisson model}
	...
	\begin{equation*}
		\begin{aligned}
			&p(x, z|r,k) = p(z|r, k) p(x|z) \pause
		\end{aligned}
	\end{equation*}
	
	Marginal 
	\begin{equation*}
		p(x|r,k) = \int_{\alert{\mathbb R_{>0}}} p(z|r, k) p(x|z) \dd z
	\end{equation*}
...
\begin{frame}{Strategy}
	...
	ELBO
	\begin{equation*}
		\E[q(\zeta|\lambda)]{f(x, \zeta)} + \Ent{q(\zeta)}
	\end{equation*}
	\uncover<7->{
...
\begin{frame}{VI optimisation problem}
	...
	To automate the search for a variational approximation $\balert{q(z)}$ we must ensure that\\
	 \begin{equation*}
