...
\end{equation*}
\pause
The first term again requires approximation by sampling,  but there is a problem -- TODO don't like this sentence
...
\node[above right= of stub](secondstub) {};
\node[circle, draw, thick, above right= of secondstub] (z) {$ z $};
\node[right = of z, xshift=-1cm] (random) {$ \sim \BerDist{b} $};

\pause

% log p(x|z)
\node[rectangle, fill=blue!20, thick, above right= of input, rounded corners, draw] (loss) {$ \log p(x|z) $};
% x -> log p(x|z)
