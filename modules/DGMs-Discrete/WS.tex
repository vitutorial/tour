...
\begin{equation*}
\begin{aligned}
&\argmin_{\theta}~ \E[p(x)]{\KL{q(z|x, \lambda)}{p(z|x, \theta)}}\\ \pause
= &\argmax_{\theta} \E[p(x)]{\ELBO(\theta, \lambda | x) - \alert{\log p(x|\theta)}} \\ \pause
% = &\argmin_{\theta}~ \E[p(x)]{\E[q(z|x, \lambda)]{\log q(z|x, \lambda) - \log \frac{p(x, z|\theta)}{p(x|\theta)}}} \\[5pt] \pause
% \approx &\argmin _{\theta}~ \E[p(x)]{\E[q(z|x, \lambda)]{\log q(z|x, \lambda) - \log p(x, z|\theta)}} \\[5pt] \pause
% \approx &\argmax_{\theta}~ \E[p(x)]{\ELBO(\theta, \lambda|x)} \\
\end{aligned}
\end{equation*} 
...
\begin{equation*}
\begin{aligned}
&\argmin_{\lambda} \mathbb E_{p(x)}\left[ \KL{\alert<1>{p(z|x, \theta)}}{q(z|x, \lambda)} \right] \\ \pause
&= \argmin_{\lambda} \mathbb E_{p(x)}\mathbb E_{p(z|x, \theta)}\left[ \log p(z|x, \theta) - \log q(z|x, \lambda) \right] \\ \pause
&\stackrel{\text{asm}}{=} \argmax_{\lambda} \mathbb E_{\alert<3>{p(x, z|\theta)}}\left[ \log q(z|x, \lambda) \right] - \underbrace{\mathbb E_{p(x,z|\theta)}\left[ \log p(z|x, \theta) \right]}_{\text{constant}}  \pause
