...
\begin{equation*}
\begin{aligned}
&\argmin_{\lambda} \KL{q(z|x, \lambda)}{\alert{p(z|x, \theta)}} \\ \pause
...
\hfill Compute $\log q(z|x, \lambda)$ and update $\lambda$
\begin{figure}
\center
\begin{tikzpicture}
\node[draw, circle, fill=orange] (z3) {$ z_{3} $};
...
\node[draw, circle, fill=gray, below left= of z1] (in1) {$ \tilde{x}_{1} $};
\node[draw, circle, fill=gray, below left=of z2] (in2) {$ \tilde{x}_{2} $}; 
\node[draw, circle, fill=gray, below right= of z2] (in3) {$ \tilde{x}_{3} $};
